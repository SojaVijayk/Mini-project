...
def book_description(book_id):
    ...
    try:
        ...
        # Also try to find the book in the database for story content
        cur = mysql.connection.cursor()
        cur.execute(
    ...
    try:
        ...
        if len(continue_reading_books) < 10:
            ...
            for book in additional_books:
                ...
                print(f"Additional book with position: {book_info['title']} (ID: {book_info['book_id']}) - Progress: {book_info['progress_percentage']}%")  # Debug log
                continue_reading_books.append(book_info)
        
        return jsonify({'success': True, 'continue_reading': continue_reading_books})
        
    except Exception as e:
        print(f"Error in continue_reading_books: {str(e)}")  # Debug log
        import traceback
        traceback.print_exc()  # Print full traceback
        return jsonify({'success': False, 'message': 'An error occurred: ' + str(e)})

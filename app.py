    ...
    try:
        ...
        if progress == 0:
            ...
            if existing_progress:
                ...
                print(f"Inserting new progress record for user_id: {session['user_id']}, book_id: {book_id}, progress: {progress}")  # Debug log
                cur.execute(
...
def continue_reading_books():
    ...
    try:
        ...
        print(f"Error in continue_reading_books route: {str(e)}")  # Debug log
        import traceback
        traceback.print_exc()  # Print full traceback
        return jsonify({'success': False, 'message': 'An error occurred: ' + str(e)})
...
def continue_reading_page_route():
    if 'loggedin' not in session:
...
# Route to get reading history for a user (including ratings and feedback)
...
def reading_history():
    ...
    try:
        ...
        print(f"Fetching reading history for user_id: {session['user_id']}")  # Debug log
        ...
        cur.execute(
            """SELECT bt.book_id, bt.title, bt.author, bt.genre, bt.ISBN,
               ...
               FROM book_table bt
               JOIN reading_progress rp ON bt.book_id = rp.book_id
               WHERE rp.user_id = %s AND rp.progress_percentage > 0
               ORDER BY rp.last_read DESC""",
            (session['user_id'],)
        )
        reading_progress_data = cur.fetchall()
        print(f"Found {len(reading_progress_data)} books with reading progress")  # Debug log
        
        # Add reading progress books to the list
        for book in reading_progress_data:
            book_info = {
                'book_id': book[0],
                'title': book[1],
                'author': book[2],
        ...
        print(f"Found {len(ratings_data)} books with ratings")  # Debug log
        
        # Add ratings to existing books or create new entries
        for book in ratings_data:
            # Check if book already exists in reading_history_books
            existing_book = None
            for existing in reading_history_books:
                if existing['book_id'] == book[0]:
                    existing_book = existing
                    break
            
            if existing_book:
                # Update existing book with rating info
                existing_book['rating'] = book[5]
                existing_book['last_read'] = book[6].isoformat() if book[6] else existing_book['last_read']
            else:
                # Add new book with rating info
                book_info = {
                    'book_id': book[0],
                    'title': book[1],
                    'author': book[2],
        ...
        print(f"Found {len(feedback_data)} books with feedback")  # Debug log
        cur.close()
        
        # Add feedback to existing books or create new entries
        for book in feedback_data:
            # Check if book already exists in reading_history_books
            existing_book = None
            for existing in reading_history_books:
                if existing['book_id'] == book[0]:
            ...
            if existing_book:
                # Update existing book with feedback info
                existing_book['feedback'] = book[5]
                existing_book['feedback_date'] = book[6].isoformat() if book[6] else existing_book['feedback_date']
                if not existing_book['last_read']:
                    existing_book['last_read'] = book[6].isoformat() if book[6] else None
            else:
                # Add new book with feedback info
                book_info = {
                    'book_id': book[0],
                    'title': book[1],
                    'author': book[2],
                    ...
                    'rating': None,
                    'feedback': book[5],
                    'feedback_date': book[6].isoformat() if book[6] else None
                }
                reading_history_books.append(book_info)
        
        # Sort by last_read date
        reading_history_books.sort(key=lambda x: x['last_read'] if x['last_read'] else '', reverse=True)
        
        # Print debug info
        for book in reading_history_books:
            print(f"Book: {book['title']} (ID: {book['book_id']}) - Progress: {book['progress_percentage']}%, Rating: {book['rating']}, Feedback: {book['feedback']}")  # Debug log
        
        return jsonify({'success': True, 'reading_history': reading_history_books})
        
    except Exception as e:
        print(f"Error in reading_history route: {str(e)}")  # Debug log
        import traceback
        traceback.print_exc()  # Print full traceback
        return jsonify({'success': False, 'message': 'An error occurred: ' + str(e)})
...
def continue_reading_main_page():
    if 'loggedin' not in session:

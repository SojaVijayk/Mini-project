...
def recommended_books():
    ...
    try:
        ...
        if preferred_author or preferred_genre:
            # Build query based on available preferences
            if preferred_author and preferred_genre:
                # Search for books by preferred author OR preferred genre
                cur.execute(
                    """SELECT book_id, title, author, genre, ISBN 
                       ...
                       ORDER BY 
                         CASE 
                           WHEN LOWER(author) LIKE LOWER(%s) THEN 1
                           WHEN LOWER(genre) LIKE LOWER(%s) THEN 2
                           ELSE 3
                         END,
                         title ASC
                       LIMIT 20""",
                    (f"%{preferred_author}%", f"%{preferred_genre}%", 
                     f"%{preferred_author}%", f"%{preferred_genre}%")
                )
            elif preferred_author:
                # Search only by preferred author
                cur.execute(
                    """SELECT book_id, title, author, genre, ISBN 
                       FROM book_table 
                       WHERE LOWER(author) LIKE LOWER(%s)
                ...
                # Search only by preferred genre
                cur.execute(
                    """SELECT book_id, title, author, genre, ISBN 
                       FROM book_table 
